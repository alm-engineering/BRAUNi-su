...
export default function OperationsberichtPage() {
  ...
  const startVoiceRecording = async () => {
    try {
      ...
      alert(
        "Fehler beim Zugriff auf das Mikrofon. Bitte Berechtigungen überprüfen."
      );
    }
  };
  ...
  const transcribeAndApplyFeedback = async (audioBlob: Blob) => {
    ...
    try {
      ...
      alert(
        "Fehler bei der Verarbeitung des Feedbacks. Bitte erneut versuchen."
      );
    } finally {
      setIsTranscribing(false);
    }
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      console.log(
        "[v0] Saved sentences to localStorage:",
        `op-${id}-sentences`,
        sentences.length
      );
    }
